...
# %%
# Import data from pre-processing step and diurnal pattern
fp1 = 'Input_Data/flow_precip_data.csv'
fp2 = 'Input_Data/Diurnal_Patterns.csv'
...
df_comb['RDII'] = np.where(df_comb['flow_lps_x'] - df_comb['flow_lps_y'] > 0, df_comb['flow_lps_x'] - df_comb['flow_lps_y'], 0)
# %%
# Create figure with two rows
fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights = [1, 4])

# Max values for axis scaling
flow_max = df_comb["flow_lps_x"].max()
...
# %%
pio.write_html(fig, "Output_Data/RDII_Plot.html")
